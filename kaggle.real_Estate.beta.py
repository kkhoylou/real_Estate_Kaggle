...
lasso_PARAM = {'alpha':[.01,.1,.5,1,5,10,15,50,100,300,500]}

grid_lasso = GridSearchCV(Lasso(), lasso_PARAM)
grid_lasso.fit(train_x, y)
print(grid_lasso.get_params)
pred_LASSO_CV = pd.DataFrame(data=np.expm1(grid_lasso.predict(test_x)),    # values
              			  index= range(TRAIN_ROWS,TRAIN_COLS), #Set Index
              			  columns=['SalePrice'])
...
rf_PARAM = {'n_estimators':[5,10,15,50,100,300,500],
...
pred_LASSO_CV.to_csv('output.csv', header=True, index_label='Id')
