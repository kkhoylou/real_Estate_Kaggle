...
alphas = [.00001,.0001,.001,.01,.1,.5,1,5,10,100,500,1000]
...
rf_PARAM = {'n_estimators':[5,10,15,50,100,300,500,1000,2000],
			'max_depth':[3,7,10,12,15]}
t4 = time.time()
grid_RF = GridSearchCV(RandomForestRegressor(), rf_PARAM)
grid_RF.fit(train_x, y)
t_rf = time.time() - t4

print("\nRandom Forest Score with CV: "+str(np.mean(cross_val_score(clf, train_x, y, cv=10)))+
         "\nTime of "+str(t_rf)+"\nWith the best Params"+str(grid_RF.best_params_))


pred_RF_CV = pd.DataFrame(data=np.expm1(grid_RF.predict(test_x)),    # values
              			  index= range(TRAIN_ROWS,TRAIN_COLS), #Set Index
              			  columns=['SalePrice'])   # 1st column as index


xgb_PARAM = {'max_depth':[3,5,8,12,15],
             'learning_rate':[.01,.1,.25,.5,1],
             'slient':[1],
             'objective':{'reg:linear'}}

grid_XGB = GridSearchCV(XGBRegressor(), xgb_PARAM)
grid_XGB.fit(xgb.DMatrix(data=train_x, label = y))
print("\n Extreme Gradient Boosting Score with CV: "+str(cv.mean())+
         "\nTime of "+str(t_xgb)+"\nWith the best Params"+str(grid_XGB.best_params_))

pred_XGB_CV = pd.DataFrame(data=np.expm1(grid_XGB.predict(test_x)),    # values
                                     index= range(TRAIN_ROWS,TRAIN_COLS), #Set Index
                                     columns=['SalePrice'])   # 1st column as index
...
all_ = (pred_lasso + pred_elastic + pred_ridge + pred_RF_CV + pred_XGB_CV)/5
all_.to_csv('output.csv', header=True, index_label='Id')
